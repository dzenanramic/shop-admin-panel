...
export default function useTable(props?: UseTableProps): ReturnType {
  const [tableSettings, setTableSettings] = useLocalStorage('tableSettings', {
    dense: !!props?.defaultDense,
    rowsPerPage: props?.defaultRowsPerPage || 10,
  });
  const [dense, setDense] = useState(
    tableSettings ? tableSettings.dense : !!props?.defaultDense,
  );
  ...
  const [rowsPerPage, setRowsPerPage] = useState(
    tableSettings ? tableSettings.rowsPerPage : props?.defaultRowsPerPage || 10,
  );
  ...
  const onChangeDense = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setTableSettings({ ...tableSettings, dense: event.target.checked });
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [],
  );
